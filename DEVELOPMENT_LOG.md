...
#### 🎯 需求起因
店家菜單調整：**牛肉便當停售，由排骨便當取代**（id 2），售價由 $110 調整為 **$95**。使用者提供排骨（炸豬排）實拍照片一張，需同步更新網站與 A4 傳單。

#### 📝 修改內容
1. **新照片**：`public/images/排骨便當.jpg`（廚師實拍炸排骨，延續第一批實拍照風格）。
2. **網站 (`src/app/[locale]/page.tsx`)**：id 2 圖片路徑改為 `排骨便當.jpg`，價格 110 → 95。
3. **五國語言檔 (`messages/*.json`)**：`mains_2_name` / `mains_2_desc` 全數改為排骨便當對應翻譯：
   | 語言 | 品名 |
   |------|------|
   | 🇹🇼 zh | 排骨便當（酥脆多汁的炸排骨） |
   | 🇹🇭 th | ข้าวกล่องหมูทอด |
   | 🇻🇳 vi | Cơm hộp sườn chiên |
   | 🇲🇲 my | ဝက်နံရိုးကြော် ထမင်းဘူး |
   | 🇰🇭 km | បាយប្រអប់សាច់ជ្រូកបំពង |
4. **A4 傳單 (`public/flyer.html`)**：牛肉便當區塊整組替換為排骨便當（照片、中文品名、$95、四國譯名並列）。
